<template>
  <div class="flex overflow-hidden rounded-2xl border border-primary/5 shadow-sm bg-white/40" style="height: calc(100vh - 9rem)">
    ...
    <!-- ── Liste conversations ──────────────────────────────────────────────── -->
    <aside class="w-80 flex-shrink-0 bg-surface-container-lowest/80 backdrop-blur-xl flex flex-col border-r border-primary/5"
      ...
      <div class="p-3 border-b border-primary/5 relative">
        ...
        <div v-if="searchResults.length"
          class="absolute left-3 right-3 top-14 bg-white rounded-xl shadow-xl border border-primary/5 z-10 overflow-hidden">
      ...
      <div class="flex-1 overflow-y-auto divide-y divide-primary/5">
        <div v-if="!loadingConvs && conversations.length === 0" class="text-center py-12 opacity-40">
          ...
          <p class="text-[10px] mt-1">Recherchez un utilisateur ci-dessus</p>
        </div>

        <div v-for="conv in conversations" :key="conv.user_id"
          @click="openConversation(conv)"
          :class="selectedUser?.user_id === conv.user_id
            ? 'bg-primary/5 border-l-4 border-primary'
            : 'hover:bg-surface-container-high'"
          class="p-4 cursor-pointer transition-all">
          <div class="flex gap-3 items-center">
            <div class="w-11 h-11 rounded-full bg-primary flex items-center justify-center text-white font-bold text-xs flex-shrink-0">
              {{ initials(conv.name) }}
            </div>
            <div class="flex-1 min-w-0">
              ...
              <p class="text-xs text-on-surface-variant truncate" :class="conv.unread ? 'font-bold text-primary' : 'font-medium'">
                {{ conv.lastMessage }}
              </p>
            </div>
    ...
    <section v-if="selectedUser" class="flex-1 flex flex-col min-w-0">
      ...
      <!-- Header -->
      <div class="p-4 flex items-center justify-between bg-surface-container-low/50 border-b border-primary/5 flex-shrink-0">
        ...
        <template v-else>
          <div v-for="msg in messages" :key="msg.id"
            ...
            <div :class="msg.sender === 'me'
                ? 'bg-primary text-white rounded-tl-2xl rounded-bl-2xl rounded-br-sm'
                : 'bg-surface-container-high text-on-surface rounded-tr-2xl rounded-br-2xl rounded-bl-sm'"
      ...
      <div class="p-4 border-t border-primary/5 flex-shrink-0">
        <div class="bg-white rounded-2xl p-2 shadow-sm flex items-end gap-2 ring-1 ring-primary/5 focus-within:ring-2 focus-within:ring-primary/20 transition-all">
          ...
          <button @click="sendMessage" :disabled="!newMessage.trim() || sending"
            class="w-10 h-10 rounded-xl bg-primary text-white flex items-center justify-center shadow-lg shadow-primary/20 hover:scale-105 active:scale-95 transition-all flex-shrink-0 disabled:opacity-40 disabled:cursor-not-allowed">
...
const sendMessage = async () => {
  ...
  try {
    ...
    // Garde le message optimistic en cas d'erreur
  } finally {
    sending.value = false
  }
}
