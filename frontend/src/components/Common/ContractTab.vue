<template>
  <div class="space-y-6">
    ...
    <div v-else-if="contracts.length === 0" class="text-center py-20 bg-white rounded-2xl border border-primary/5">
      <span class="material-symbols-outlined text-5xl text-on-surface-variant/30 block mb-3">contract</span>
    ...
    <div v-else class="space-y-4">
      <div v-for="c in contracts" :key="c.id"
        class="bg-white rounded-2xl border border-primary/5 p-6 shadow-sm hover:shadow-md transition-all">

        <div class="flex justify-between items-start gap-4">
          <div class="flex-1">
            <div class="flex items-center gap-3 mb-2">
              <span :class="statusClass(c.status)" class="text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full">
          ...
          <div class="text-right flex-shrink-0">
            ...
            <p class="text-xs text-on-surface-variant mt-1">Commission : {{ Number(c.commission).toLocaleString('fr-FR') }} MAD</p>
          </div>
        </div>

        <!-- Actions -->
        <div v-if="c.status === 'active'" class="mt-4 flex gap-3 justify-end">
          <button v-if="role === 'client'" @click="complete(c)"
            class="px-5 py-2 bg-primary text-white text-xs font-bold rounded-full hover:brightness-110 transition-all flex items-center gap-1">
            <span class="material-symbols-outlined text-sm">check_circle</span>
            Valider la livraison
          </button>
          <button v-if="role === 'client'" @click="refund(c)"
            class="px-5 py-2 bg-secondary/10 text-secondary text-xs font-bold rounded-full hover:bg-secondary hover:text-white transition-all flex items-center gap-1">
...
const endpoint = computed(() =>
  props.role === 'freelancer'
    ? 'http://localhost:8000/api/contracts/freelancer'
    : 'http://localhost:8000/api/contracts/client'
)
...
const load = async () => {
  ...
  try {
    ...
    contracts.value = props.role === 'freelancer'
      ? [
          { id: 1, mission: { title: 'Rénovation Salon Riadh' }, client: { name: 'Mehdi El Fassi' }, amount: 8500, commission: 425, status: 'completed', created_at: new Date().toISOString() },
          { id: 2, mission: { title: 'Site web E-Commerce' }, client: { name: 'Sarah M.' }, amount: 12000, commission: 600, status: 'active', created_at: new Date().toISOString() },
        ]
      : [
          { id: 1, mission: { title: 'Rénovation Salon Riadh' }, freelancer: { name: 'Yasmine B.' }, amount: 8500, commission: 425, status: 'completed', created_at: new Date().toISOString() },
          { id: 2, mission: { title: 'Identité Visuelle' }, freelancer: { name: 'Omar K.' }, amount: 5500, commission: 275, status: 'active', created_at: new Date().toISOString() },
        ]
  } finally {
    loading.value = false
  }
...
const complete = async (contract) => {
  try {
    await axios.post(`http://localhost:8000/api/contracts/${contract.id}/complete`, {}, { headers: headers.value })
...
const refund = async (contract) => {
  ...
  try {
    await axios.post(`http://localhost:8000/api/contracts/${contract.id}/refund`, {}, { headers: headers.value })
